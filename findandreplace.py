...
for repository in repository_list:
    print("Modifying repo " + repository[0] + "...")
    savedir = repository[0]
    ticket_id = repository[1]
    service_repo = repository[2]
    path_to_repository = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      repositories_directory + savedir)
    ...
    for checked_file in file_list:
        ...
        repo_file = pathlib2.Path(path_to_repository_file)
        print(path_to_repository_file)
        if repo_file.exists():
